...
const ColegiosTab=()=>{
  ...
  const crear=async()=>{
    // v2.19: la contraseña del director la escribe el Superadmin. Antes había
    // un default 'Cambiar123' que creaba colegios con una clave conocida.
    if(!edit&&!nuevo.admin_password.trim()){setError('Escriba la contraseña inicial del director. El sistema le pedirá cambiarla en su primer acceso.');return;}
    ...
    try{
      const r=await api.post('/superadmin/colegios',{
        ...
        admin_password:nuevo.admin_password,
      });
...
// ===== USUARIOS =====
const UsuariosTab=()=>{
  const [colegios,setColegios]=useState<Colegio[]>([]);const [sel,setSel]=useState<number|null>(null);const [usuarios,setUsuarios]=useState<UsuarioColegio[]>([]);const [loading,setLoading]=useState(false);const [error,setError]=useState('');const [success,setSuccess]=useState('');const [resetM,setResetM]=useState<UsuarioColegio|null>(null);const [newPw,setNewPw]=useState('');const [crearM,setCrearM]=useState(false);const [nu,setNu]=useState({username:'',nombre:'',apellido:'',role:'profesor',email:'',password:''});
  ...
  const resetPw=async()=>{if(!resetM||!sel)return;
    if(!newPw.trim()){setError('Escriba la contraseña temporal. El usuario deberá cambiarla en su próximo acceso.');return;}try{await api.post(`/superadmin/colegios/${sel}/reset-password`,{usuario_id:resetM.id,password:newPw});setSuccess(`Password reseteado para ${resetM.nombre_completo}`);setResetM(null);}catch(e:any){setError(e.response?.data?.error||'Error');}};
  ...
  const crearUsr=async()=>{if(!sel||!nu.username||!nu.nombre){setError('Username y nombre requeridos');return;}
    if(!nu.password.trim()){setError('Escriba la contraseña inicial. El sistema le pedirá cambiarla en su primer acceso.');return;}try{const r=await api.post(`/superadmin/colegios/${sel}/crear-usuario`,nu);setSuccess(`Usuario ${r.data.username} creado. Debe cambiar la contraseña en su primer acceso.`);setCrearM(false);setNu({username:'',nombre:'',apellido:'',role:'profesor',email:'',password:''});load(sel);}catch(e:any){setError(e.response?.data?.error||'Error');}};
  return(
    <div className="space-y-4">
      <Alerts error={error} success={success} onCE={()=>setError('')} onCS={()=>setSuccess('')}/>
